...
"""
    ...
    async def _transcribe_with_whisper(self, url: str) -> Optional[str]:
        ...
        try:
            ...
            print(f"🔍 Running Whisper CLI command...")
            result = subprocess.run(
            ...
            # Whisper outputs to stdout, so parse it from there
            # The output is in SRT format with timestamps, extract just the text
            transcript_lines = []
            for line in result.stdout.split('\n'):
                ...
                # Check if line has timestamp format [00:00.000 --> 00:11.000]
                if '[' in line and '-->' in line:
                    # Extract text after the timestamp pattern
                    # Pattern: [00:00.000 --> 00:11.000]  The challenge is sooner or later.
                    try:
                        # Split by ']' to get text after timestamp
                        parts = line.split(']')
                        if len(parts) > 1:
                            text = parts[1].strip()
                            if text:
                                transcript_lines.append(text)
                                print(f"📝 Extracted from line: {text}")
                    except:
                        pass
                    continue
            ...
            print(f"❌ Whisper transcription failed - no valid transcript found")
            return None
            ...
            try:
                ...
                video_patterns = {
                    ...
                    'instagram_reels': [
                        ...
                        r'https?://(?:www\.)?instagr\.am/reel/[^\s]+',
                        r'https?://instagr\.am/reel/[^\s]+',
                    ]
                }
